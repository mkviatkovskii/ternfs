...
struct ShardShared {
    ...
    std::atomic<uint64_t> gatedStaleReads;
    ...
    inline bool readStale() const {
        if (options.readStalenessThreshold == 0) { return false; }
        return (ternNow() - shardDB.lastAppliedEntryTime()) > options.readStalenessThreshold;
    }
...
private:
    ...
    TernTime _lastHeartbeatEmitted{0};
...
public:
    ...
    void _maybeEmitHeartbeat() {
        if (_shared.options.heartbeatEmissionInterval == 0 || !_isLogsDBLeader || _shared.options.isProxyLocation()) {
            return;
        }
        auto now = ternNow();
        if (!_shardEntries.empty() || !_inFlightEntries.empty()) {
            _lastHeartbeatEmitted = now;
            return;
        }
        if (now - _lastHeartbeatEmitted < _shared.options.heartbeatEmissionInterval) {
            return;
        }
        _lastHeartbeatEmitted = now;
        auto& entry = _shardEntries.emplace_back();
        entry.time = now;
        entry.body.setHeartbeat();
        entry.idx = _currentLogIndex + _inFlightEntries.size() + _shardEntries.size();
        // msg.id == 0 routes it through the request-less apply branch.
        ShardReq heartbeatReq;
        heartbeatReq.msg.id = 0;
        _logIdToShardRequest.insert({entry.idx.u64, std::move(heartbeatReq)});
    }
    ...
    void _processLinkReadRequest(const ShardReq& req) {
        if (unlikely(_shared.readStale())) {
            _shared.gatedStaleReads.fetch_add(1, std::memory_order_relaxed);
            return;
        }
        ShardRespMsg resp;
        resp.id = req.msg.id;
...
public:
        ...
        _sendMetricsAlert(XmonAppType::DAYTIME, 5_mins)
    {
        _stats = {
            ...
            {"eggsfs_shard_reads_gated_stale", &_shared.gatedStaleReads},
            // error counts
    ...
    virtual bool periodicStep() {
        ...
        {
            newMeassurement("eggsfs_shard_staleness_lag");
            _metricsBuilder.fieldU64("ns", (now - _shared.shardDB.lastAppliedEntryTime()).ns);
            _metricsBuilder.timestamp(now);
        }
